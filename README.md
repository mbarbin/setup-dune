...
| `dune-digest`        | expected digest of the dune binary for integrity verification   |                         |
...
`dune-digest` specifies the expected digest of the dune binary, using the syntax
`sha256:<hex-encoded-hash>`. When provided for a stable version (i.e. not
`nightly`, `dev`, or `latest`), the binary is automatically cached across runs
using the digest as part of the cache key. On cache hit, the digest is
re-verified but the attestation check is skipped, since the binary was already
verified when it was first cached. Downloaded binaries are always verified
against the [ocaml-dune/binary-distribution](https://github.com/ocaml-dune/binary-distribution)
attestation using `gh attestation verify`.
